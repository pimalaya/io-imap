...
impl ImapCoroutine for ImapMailboxUnsubscribe {
    ...
    fn resume(
        ...
        if let Some(bye) = bye {
            return ImapCoroutineState::Complete(Err(ImapMailboxUnsubscribeError::Bye(
                bye.text.to_string(),
            )));
        }

        let Some(Tagged { body, .. }) = tagged else {
            return ImapCoroutineState::Complete(Err(ImapMailboxUnsubscribeError::MissingTagged));
        };

        match body.kind {
            StatusKind::Ok => ImapCoroutineState::Complete(Ok(())),
            StatusKind::No => ImapCoroutineState::Complete(Err(ImapMailboxUnsubscribeError::No(
                body.text.to_string(),
            ))),
            StatusKind::Bad => ImapCoroutineState::Complete(Err(ImapMailboxUnsubscribeError::Bad(
                body.text.to_string(),
            ))),
        }
    }
