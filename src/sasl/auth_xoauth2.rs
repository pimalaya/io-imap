...
mod tests {
    ...
    fn ir_success_returns_ok() {
        ...
        let mut xoauth2 = ImapAuthXoauth2::new("user@example.org", "oauth-token", opts);
        let mut frag = Fragmentizer::new(50 * 1024 * 1024);

        let bytes = expect_wants_write(&mut xoauth2, &mut frag, None);
        let line = str::from_utf8(&bytes).expect("utf8 command");
        let tag = first_word(line);
        assert!(line.contains("AUTHENTICATE XOAUTH2 "));

        expect_wants_read(&mut xoauth2, &mut frag);

        let reply = format!("{tag} OK AUTHENTICATE completed\r\n");
        expect_complete_ok(&mut xoauth2, &mut frag, reply.as_bytes());
    }
    ...
    fn ir_invalid_token_returns_no_with_error() {
        ...
        let mut xoauth2 = ImapAuthXoauth2::new("user@example.org", "expired-token", opts);
        let mut frag = Fragmentizer::new(50 * 1024 * 1024);
        ...
        let ack = expect_wants_write(&mut xoauth2, &mut frag, Some(challenge.as_bytes()));
        assert_eq!(b"\r\n", &*ack);

        expect_wants_read(&mut xoauth2, &mut frag);

        let reply = format!("{tag} NO SASL authentication failed\r\n");
        let err = expect_complete_err(&mut xoauth2, &mut frag, reply.as_bytes());
    ...
    fn ir_tagged_bad_returns_bad_error() {
        ...
        let mut cor = ImapAuthXoauth2::new("user@example.org", "oauth-token", opts);
        let mut frag = Fragmentizer::new(50 * 1024 * 1024);

        let bytes = expect_wants_write(&mut cor, &mut frag, None);
        let tag = first_word(str::from_utf8(&bytes).expect("utf8 command"));

        expect_wants_read(&mut cor, &mut frag);

        let reply = format!("{tag} BAD AUTHENTICATE not enabled\r\n");
        let err = expect_complete_err(&mut cor, &mut frag, reply.as_bytes());
    ...
    fn non_ir_success_returns_ok() {
        ...
        let creds = expect_wants_write(&mut xoauth2, &mut frag, Some(b"+ \r\n"));
    ...
    fn non_ir_invalid_token_returns_no_with_error() {
        ...
        expect_wants_read(&mut xoauth2, &mut frag);
        expect_wants_write(&mut xoauth2, &mut frag, Some(b"+ \r\n"));
        expect_wants_read(&mut xoauth2, &mut frag);
