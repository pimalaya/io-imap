...
impl TokioSession {
    ...
    async fn watch(
        ...
        let mut watcher = ImapMailboxWatch::new(&self.capability, mailbox, flag.clone(), opts);
        let mut buf = [0u8; READ_BUFFER_SIZE];
        ...
        loop {
            match watcher.resume(&mut self.fragmentizer, arg.as_deref()) {
                ...
                // NOTE: this watch idles, so it never asks to wait. A
                // polling one would sleep here, racing the shutdown
                // the same way the read below does.
                ImapCoroutineState::Yielded(ImapMailboxWatchYield::WantsWait) => arg = None,
